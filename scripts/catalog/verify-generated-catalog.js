...
if (!/^[a-z][a-z0-9_]*$/.test(slug || '')) throw new Error('Usage: verify-generated-catalog.js <slug> [--remote] [--database]');
const roster = require(`./${slug}.roster.json`);
const dir = path.resolve(__dirname, '../../assets/catalog', slug);
...
async function main() {
  ...
  if (process.argv.includes('--database')) {
    ...
    try {
      const { rows } = await client.query('SELECT name,url FROM image WHERE category=$1 AND user_id IS NULL ORDER BY name', [slug]);
      assert.deepEqual(rows.sort((a,b) => a.name < b.name ? -1 : a.name > b.name ? 1 : 0), manifest.images.map(i => ({ name: i.name, url: i.url })).sort((a,b) => a.name < b.name ? -1 : a.name > b.name ? 1 : 0));
      const hidden = await client.query('SELECT 1 FROM category_setting WHERE slug=$1 AND visible=false', [slug]);
